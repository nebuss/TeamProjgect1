...
b = (bf_covid_sum.loc['total', :]) / 49200
b

# 그래프 만들기
sns.barplot(data = b)
plt.title('Covid-19 이전(2018~2019년) 1인 평균 여행 지출액')
plt.ylabel('person_cost')
plt.show()
plt.clf()
------------------------------------------------------------------
...
d = (du_covid_sum.loc['total', :]) / 51000
d
...
af = af_covid.transpose()
af['total'] = af['sum_22']
af_covid_sum = af.transpose()
a = (af_covid_sum.loc['total', :]) / 51600 #표본 수 51,600
